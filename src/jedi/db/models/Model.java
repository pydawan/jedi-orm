...
public class Model implements Comparable<Model>, Serializable {
    ...
    /**
     * Stores the invoker object in the database.
     * 
     * @author Thiago Alexandre Martins Monteiro
     * @version 1.0
    ...
    public void insert() {
        ...
        try {
            ...
            for (Field field : this.getClass().getDeclaredFields()) {
                ...
                if (field.getType().toString().endsWith("String")) {
                    ...
                    if (field.get(this) != null) {
                        ...
                        // Substituindo ' por \' para evitar erro de sintaxe no
                        // SQL.
                    ...
                    if (date != null) {
                        ...
                        if (date_field_annotation != null) {
                            ...
                            if (date_field_annotation.default_value().trim().equals("")) {
                                ...
                                values += String.format(date_field_annotation.default_value().trim().equalsIgnoreCase("NULL") ? "%s, " : "'%s', ",
                                        date_field_annotation.default_value().trim());
                            }
                    ...
                    if (foreign_key_annotation != null) {
                        ...
                        // 1 - Obtendo a referencia para o campo do modelo
                        // atual.
                        // 2 - Recuperando o valor do id.
                        // OBS: Tratar o retorno de toString() do modelo e muito
                        // perigoso uma vez que
                        // o usuario pode ter a possibilidade de mudar a
                        // representacao do objeto e o codigo
                        // que faz o tratamento do retorno e altamente
                        // dependente do formato do retorno.
                        ...
                        // String id = "";
                        // Lancar excessao de campo nao existente informando
                        // qual o campo.
                        // Se o atributo pais nao for configurado em Uf por
                        // exemplo, ocorrera NullPointerException.
            ...
            // Se o modo debug ou log estiver habilitado mostrar o SQL gerado na
            // saída padrão.
            ...
            // Verificar se o usuario optou por habilitar o recurso de
            // auto-incremento.
            Manager manager = new Manager(this.getClass());

            // Obtendo o id do último registro inserido e atribuindo ao id do
            // objeto atual.
                ...
                for (Field field : this.getClass().getDeclaredFields()) {
                    ...
                    if (field.getType().getSuperclass() != null && field.getType().getSuperclass().getSimpleName().equals("Model")) {
                        ...
                        if (foreign_key_annotation != null && !foreign_key_annotation.references().isEmpty()) {
                            ...
                            fields_and_values += String.format("%s_id = ", field.getType().getSimpleName().replaceAll("([a-z0-9]+)([A-Z])", "$1_$2")
                                    .toLowerCase());
                        }

                    } else {

                        // fields_and_values += String.format("%s = ",
                        // field.toString().substring(field.toString().lastIndexOf('.')
                        // + 1).toLowerCase() );
                    ...
                    if (field.getType().toString().endsWith("String")) {
                        ...
                        "'%d-%02d-%02d %02d:%02d:%02d', ", date.getYear() + 1900, date.getMonth() + 1, date.getDate(), date.getHours(), date.getMinutes(),
                                date.getSeconds());

                    } else {

                        // Verificando se o campo foi anotado como chave
    ...
    public void save() {
        ...
        // Depois se necessario tratar desativacao do recurso de
        // auto-incremento.
        try {
    ...
    public void delete() {
        ...
        try {
            ...
            // Informando que o modelo não se encontra persistido no banco de
            // dados.
                ...
                if (f.getType().getSuperclass() != null && f.getType().getSuperclass().getName().equals("jedi.db.models.Model")) {
                    ...
                    if (!((List) f.get(this)).isEmpty()) {
                        ...
                        if (table_annotation != null && !table_annotation.name().trim().isEmpty()) {
                            ...
                            xml_child_open_tag = String.format("\n%s<%ss>", identation_to_child_elements, ((List) f.get(this)).get(0).getClass()
    ...
    public String to_extense_xml(int i) {
        ...
        // A IMPLEMENTAR: Fazer o tratamento de caracteres especiais como < ou >
        // no conteudo dos atributos
                ...
                if (f.getType().getSuperclass() != null && f.getType().getSuperclass().getName().equals("jedi.db.models.Model")) {
                    ...
                    if (!((List) f.get(this)).isEmpty()) {
                        ...
                        if (table_annotation != null && !table_annotation.name().trim().isEmpty()) {
                            ...
                            xml_child_open_tag = String.format("%s<%ss>", identation_to_child_elements, ((List) f.get(this)).get(0).getClass().getSimpleName()
                                    .toLowerCase());

                            xml_child_close_tag = String.format("\n%s</%ss>\n", identation_to_child_elements, ((List) f.get(this)).get(0).getClass()
                                    .getSimpleName().toLowerCase());
